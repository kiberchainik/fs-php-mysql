	...
	class VacanciesModel extends Model {
       ...
       public static function Instance() {
        ...
        public function getListCompany($limit = '') {
            return DBClass::Instance()->select(
                'user_date as ud, user_type_company as utc', 
                array('ud.user_id, ud.company_name, ud.user_img, ud.company_link, ud.about'), 
                'ud.type_person = 4 and utc.id_type_company = 3 and ud.user_id = utc.id_user',
                array(),
                '',
                'company_name',
                '',
        ...
        public function GetCategoryListWithoutParetnIdWithCountVacanciesForCategory($loc = '', $limit = '') {
            return DBClass::Instance()->select(
                'categoryvacancies as cv, categoryvacancies_description as cvd',
                array('cv.id, cvd.seo, cv.parent_id, cvd.title, (select count(id) from '.DBClass::Instance()->config['db_pref'].'vacancies where id_category = cv.id) as vacancies_count'),
                'cvd.category_id = cv.id AND cvd.lang_id = :lang_id',
                array('lang_id' => (int)$_SESSION['lid']),
                '',
                'title',
        ...
        public function getUserVacanciesCount ($id_user, $const = 5) {
       ...
       public function getVacanciesPage ($pagenum, $const = 5, $subCatId = '', $sort = 'title') {
            ...
            return DBClass::Instance()->select(
                ...
                array('v.id, v.seo, v.title, v.date_add as date, v.short_desc, ud.user_id, ud.name, ud.lastname, ud.user_img, cd.title as category_title, cd.category_id as category_id'),
                'valid_status = 1  and v.id_category = cd.category_id '.$subCatIdWhere,
				$params,
                '',
                $sort,
                '',
       ...
       public function getVacanciesCount ($const = 5, $subCatId = '') {
            ...
            $count = DBClass::Instance()->getCount(
                ...
                'v.valid_status = 1 and v.id_category = cvd.category_id and cvd.lang_id = :lang '.$subCatIdWhere,
                $params,
                'v.id'
            );
       ...
       public function getVacanciesByLocation ($loc) {
            return DBClass::Instance()->search_like(
                'SELECT v.location, count(v.location) as num FROM tori_vacancies as v WHERE MATCH(v.location) AGAINST (:loc IN BOOLEAN MODE) and v.valid_status = 1 GROUP BY v.location HAVING count(v.location)>1',
				array('loc' => $loc.'*')
            );
        }
       ...
       public function getListProfileVacancies($id) {
        ...
        public function getTottalVacanciesOfBranchCount ($id_branch) {
            $count = DBClass::Instance()->getCount(
                'vacancies',
                'valid_status = 1  and id_filial = :id_filial',
                array('id_filial' => $id_branch)
            );
            
            return $count['numCount'];
       }
        
        public function getVacanciesOfBranchCount ($id_branch, $const = 5, $subCatId = '') {
            if(!empty($subCatId)){
                $subCatIdWhere = 'and id_category = :id_category and id_filial = :id_filial';
                $params = array('id_filial' => $id_branch, 'id_category' => $subCatId);
            } else {
                $subCatIdWhere = ' and id_filial = :id_filial';
                $params = array('id_filial' => $id_branch);
            }
       ...
       public function getVacanciesOfBranchPage ($pagenum, $id_branch, $const = 5, $subCatId = '', $sort = 'date_add') {
            $offset = (int)($pagenum-1)*$const;
            $offset = ($offset == 0)?1:$offset;
            ...
            if(!empty($subCatId)){
                $subCatIdWhere = 'and v.id_category = :id_category and v.id_filial = :id_filial';
                $params = array('id_filial' => $id_branch, 'lang' => (int)$_SESSION['lid'], 'id_category' => $subCatId);
            } else {
                $subCatIdWhere = ' and v.id_filial = :id_filial';
                $params = array('id_filial' => $id_branch, 'lang' => (int)$_SESSION['lid']);
            }
       ...
       public function getDataVacancies ($id) {
            return DBClass::Instance()->select(
                'categoryvacancies_description as cvd, vacancies as v left join '.DBClass::Instance()->config['db_pref'].'users as u on v.id_user = u.id left join '.DBClass::Instance()->config['db_pref'].'user_date as ud on v.id_user = ud.user_id  left join '.DBClass::Instance()->config['db_pref'].'branch as ub on ud.user_id = ub.id_user and ub.id = v.id_filial',
                array('v.*, cvd.title as category_title, u.login, u.email, ud.address, ud.name, ud.patent, ud.company_name, ud.company_link, ud.lastname, ud.mobile, ud.user_img, ud.type_person, ub.name_company as filial_name, ub.adres as filial_address, ub.phone as filial_phone, ub.email as filial_email, ub.url_company as filial_url_company, ub.img as filial_img'),
                '(v.id = :id or v.seo = :id) and cvd.category_id = v.id_category and cvd.lang_id = :lang',
       ...
       public function getAuthorId($id) {
            return DBClass::Instance()->select(
                'vacancies as v, users as u',
                array('v.id_user, u.email'),
                'v.id = :id and v.id_user = u.id',
                array('id' => $id),
                '',
                'email',
                '',
       ...
       public function getListVacanciesOfUser($id) {
        ...
        public function getListVacanciesForUser($pagenum, $id, $const = 5, $sort = 'title', $order = true) {
            $offset = (int)($pagenum-1)*$const;
            $offset = ($offset == 0)?1:$offset;
            ...
            return DBClass::Instance()->select(
                'vacancies as v, branch as b',
                array('v.id, v.title, v.seo, v.short_desc, v.date_add, v.views, v.show_status, v.valid_status, b.name_company'),
				'v.id_user = :id and v.id_filial = b.id',
				array('id' => (int)$id),
				'',
				$sort,
				$order,
