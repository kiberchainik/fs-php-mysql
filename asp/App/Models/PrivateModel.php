	...
	class PrivateModel extends Model {
       ...
       public function getNoteData () {
        ...
        public function AddNewCategoryVacancie ($Data, $cd) {
            ...
            foreach($cd as $k => $v){
                DBClass::Instance()->insert('categoryvacancies_description', array('category_id' => (int)$category_id, 'lang_id' => (int)$k, 'title' => $v['title'], 'keywords' => $v['keywords'], 'seo' => $v['seoTitle'], 'description' => $v['description']));
            }
        }
        ...
        public function GetCategoryVacancieData($id) {
            ...
            $query = DBClass::Instance()->select(
                ...
                array(DBClass::Instance()->config['db_pref'].'categoryvacancies_description.title, '.DBClass::Instance()->config['db_pref'].'categoryvacancies_description.description, '.DBClass::Instance()->config['db_pref'].'categoryvacancies_description.keywords, '.DBClass::Instance()->config['db_pref'].'categoryvacancies_description.seo, '.DBClass::Instance()->config['db_pref'].'categoryvacancies_description.lang_id'),
       ...
       public function UpdateCategoryVacancie ($data, $id, $category_description) {
            ...
            foreach($category_description as $k => $v){
                DBClass::Instance()->insert('categoryvacancies_description', array('category_id'  => (int)$id, 'lang_id' => (int)$k, 'title' => $v['title'], 'keywords' => $v['keywords'], 'seo' => $v['seoTitle'], 'description' => $v['description']));
            }
        }
       ...
       public function getVacanciesPage ($pagenum, $const = 5) {
            ...
            return DBClass::Instance()->select(
                ...
                array('v.id, v.id_user, v.title, v.date_add, v.short_desc, v.location, v.valid_status, ud.user_id, ud.name, ud.lastname, ud.user_img, cd.title as category_title, cd.category_id as category_id, u.email'),
                'v.id_user = u.id',
				array(),
                '',
                array('valid_status, title', 'ASC, ASC'),
                '',
                $const,
                $offset,
                '2'
       ...
       public function getFieldsGroup () {
        ...
        public function addField ($data, $id_group) {
            ...
            foreach ($data['placeholder'] as $k => $v) {
                DBClass::Instance()->insert('fields_placeholder', array('placeholder' => $v['name'], 'id_lang' => (int)$k, 'id_field' => (int)$last_id));
            }
            
            foreach ($data['value'] as $id => $val) {
                foreach($val as $k => $v) DBClass::Instance()->insert('fields_value', array('value' => $v['name'], 'id_lang' => (int)$k, 'id_value' => (int)$id, 'id_field' => (int)$last_id));
            }
        ...
        public function getFieldData ($id) {
            $fieldDate = DBClass::Instance()->select(
                'fields_data 
                LEFT JOIN '.DBClass::Instance()->config['db_pref'].'fields_placeholder ON '.DBClass::Instance()->config['db_pref'].'fields_placeholder.id_field = '.DBClass::Instance()->config['db_pref'].'fields_data.id',
            ...
            $fields_value = DBClass::Instance()->select(
                'fields_value', 
                array('*'), 
                'id_field = :id_field',
                array('id_field' => (int)$id),
            ...
            if(!empty($fields_value)) {
                ...
                foreach($fields_value as $v) {
                    if($fields_value[0]['id_value'] == $v['id_value']) $num ++;
                }
                $fields_value = array_chunk($fields_value, $num);
                
                foreach ($fields_value as $id_val => $sub_arr) {
                    foreach ($sub_arr as $v) {
                        $data['field_value'][$v['id_value']][$v['id_lang']] = array('name' => $v['value']);
                    }
                }
        ...
        public function UpdFieldDate ($data, $id_group, $id) {
            ...
            foreach ($data['placeholder'] as $k => $v) {
                DBClass::Instance()->insert('fields_placeholder', array('placeholder' => $v['name'], 'id_lang' => (int)$k, 'id_field' => (int)$id));
            }
            
            DBClass::Instance()->deleteElement('fields_value', array('id_field' => (int)$id));
            foreach ($data['value'] as $id_value => $val) {
                foreach($val as $k => $v) DBClass::Instance()->insert('fields_value', array('value' => $v['name'], 'id_lang' => (int)$k, 'id_value' => (int)$id_value, 'id_field' => (int)$id));
            }
       ...
       public function UpdatePage ($data, $id, $page_description) {
        ...
        /*----- Private API users -----*/
        public function getAPIUsersCount ($const = 5) {
            $count = DBClass::Instance()->getCount(
                'api_access',
                '',
                array(),
       ...
       public function getAPIUserList ($pagenum, $const = 5) {
            $offset = (int)($pagenum-1)*$const;
            $offset = ($offset == 0)?1:$offset;
            ...
            return DBClass::Instance()->select(
                'api_access LEFT JOIN '.DBClass::Instance()->config['db_pref'].'user_date ON '.DBClass::Instance()->config['db_pref'].'user_date.user_id = '.DBClass::Instance()->config['db_pref'].'api_access.user_id',
                array(DBClass::Instance()->config['db_pref'].'api_access.id, '.DBClass::Instance()->config['db_pref'].'api_access.email, '.DBClass::Instance()->config['db_pref'].'api_access.user_id, '.DBClass::Instance()->config['db_pref'].'user_date.name, '.DBClass::Instance()->config['db_pref'].'user_date.lastname, '.DBClass::Instance()->config['db_pref'].'user_date.mobile, '.DBClass::Instance()->config['db_pref'].'user_date.patent, '.DBClass::Instance()->config['db_pref'].'user_date.user_img'),
                '',
                '',
                '',
                'login',
       ...
       public function getUserList ($pagenum, $const = 5) {
            ...
            return DBClass::Instance()->select(
                ...
                array(DBClass::Instance()->config['db_pref'].'users.id, '.DBClass::Instance()->config['db_pref'].'users.validStatus, '.DBClass::Instance()->config['db_pref'].'users.login, '.DBClass::Instance()->config['db_pref'].'users.email, '.DBClass::Instance()->config['db_pref'].'users.onlineSatus, '.DBClass::Instance()->config['db_pref'].'users.date_reg, '.DBClass::Instance()->config['db_pref'].'user_date.name, '.DBClass::Instance()->config['db_pref'].'user_date.lastname, '.DBClass::Instance()->config['db_pref'].'user_date.mobile, '.DBClass::Instance()->config['db_pref'].'user_date.patent, '.DBClass::Instance()->config['db_pref'].'user_date.user_img, '.DBClass::Instance()->config['db_pref'].'user_type_description.name as nameType, (select count(id) from '.DBClass::Instance()->config['db_pref'].'adverts where '.DBClass::Instance()->config['db_pref'].'adverts.idUser = '.DBClass::Instance()->config['db_pref'].'users.id) as numUserAdverts'),
                '',
                '',
                '',
                'login',
