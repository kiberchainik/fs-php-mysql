                ...
                <div class="row">
                    <div class="private_wrapp">
                        ...
                        <?php if ($upd_message): ?>
                            <p><?=$upd_message?></p>
                        <?php endif ?>
                        <form method="post" action="/newbanner" enctype="multipart/form-data">
                            <div class="form-group">
                                <label for="link">Ссылка</label>
                                <input id="link" class="form-control col-md-7 col-xs-12" name="link" type="text" />
                            </div>
                            <div class="form-group">
                                <div class="col-xs-12">
                                    <table class="table">
                                ...
                                <div class="tab-content">
                                    ...
                                    <div class="tab-pane <?=($k == '0')?'active':''?>" id="lang<?=$v['id']?>">
                                        <div class="form-group">
                                            ...
                                            <input id="title_<?=$v['id']?>" name="bd[<?=$v['id']?>][title]" placeholder="<?=$v['title']?>" class="form-control col-md-7 col-xs-12" type="text" />
                                        </div>
                                        <div class="form-group">
                                            <label for="text_<?=$v['id']?>"></label>
                                            <textarea id="text_<?=$v['id']?>" name="bd[<?=$v['id']?>][text]"></textarea>
                                            <script>
                                            	ClassicEditor
