            ...
            <div class="col-md-9 col-xs-12 col-sm-12">
                <div class="row">
                    <div class="private_wrapp">
                        <form action="/provinces/edit/<?=$p_data['id']?>" method="post" id="demo-form2">
                            <div class="col-xs-12">
                            ...
                            <div class="col-xs-12">
                                <input type="hidden" value="<?=$p_data['id_region']?>" id="id_pr" />
                                ...
                                <div class="tab-content">
                                    ...
                                    <div class="tab-pane <?=($k == '0')?'active':''?>" id="lang<?=$v['id']?>">                            
                                        <div class="form-group">
                                            ...
                                            <input id="title_<?=$v['id']?>" name="p_data[<?=$v['id']?>][name]" placeholder="<?=$v['title']?>" value="<?=$p_data['p_data'][$v['id']]['name']?>" class="form-control col-md-7 col-xs-12" type="text" />
                                        </div>
                                    </div>
                                    <?php endforeach ?>
                                </div>
            ...
            if(c_id != '0') {
                $.ajax({
                	...
                	success: function(data){
                        $.map(data, function(i) {
                            if(i.id == $('#id_pr').val()) {
                                $('#region-list').append('<option value="'+i.id+'" selected>'+i.name+'</option>');
                            } else {
