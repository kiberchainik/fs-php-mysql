	...
	class Rest {
		...
		public function validateToken() {
			try {
				...
				$payload = JWT::decode($token, $this->param['secret_key'], ['HS256']);

				$stmt = $this->dbConn->prepare("
                    SELECT aa.email, aa.pass, aa.secret_key, aa.user_id, u.validStatus, ud.type_person 
                    FROM tori_users as u, tori_api_access as aa, tori_user_date as ud 
                    WHERE aa.user_id = ud.user_id AND aa.user_id = u.id AND aa.user_id = :userId");
		...
		public function processApi() {
			try {
				$api = new API;
                //$this->secret_key = $api->secret_key;
		...
		public function returnResponse($code, $data) {
			header("content-type: application/json");
            http_response_code($code);
			echo json_encode(['resonse' => ['status' => $code, "result" => $data]]);
			exit;
		}
